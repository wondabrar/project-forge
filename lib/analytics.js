...
// Uses anatomy-weighted distribution: a Squat doesn't only train legs, it
// distributes its volume across quads (primary, 1.0) + glutes (0.5) + hams
// (0.25) + core (0.3) + calves (0.15). The weights ensure secondaries are
// represented but never inflated to the point of replacing direct work.
//
// Display buckets are aggregated via DISPLAY_BUCKET (e.g. Front/Side/Rear
// Delts → "Shoulders", Biceps/Triceps/Forearms → "Arms"). Granular tracking
// happens internally; chart shows 9 buckets.
//
// Returns [{ weekStart, byMuscle: { "Quads": { sets, volume }, ... } }]
export function weeklyVolume(history) {
  ...
  for (const rec of history || []) {
    ...
    for (const block of rec.blocks || []) {
      for (const ex of block.exercises || []) {
        // Compute total volume for this exercise across all sets
        let exVolume = 0;
        let exSets = 0;
        for (const s of ex.sets || []) {
          exSets += 1;
          if (s.volume != null) {
            ...
            if (load && reps) exVolume += load * reps;
          }
        }
        if (exSets === 0) continue;

        // Distribute volume + set count across muscles by anatomy weights
        const volByMuscle  = distributeAcrossMuscles(ex.name, exVolume, ex.muscle);
        const setsByMuscle = distributeAcrossMuscles(ex.name, exSets,   ex.muscle);

        // Aggregate to display buckets and add to the week's tally
        for (const [muscle, vol] of Object.entries(volByMuscle)) {
          const bucket = DISPLAY_BUCKET[muscle] || muscle;
          if (!byWeek[weekStart][bucket]) byWeek[weekStart][bucket] = { sets: 0, volume: 0 };
          byWeek[weekStart][bucket].volume += vol;
        }
        for (const [muscle, sets] of Object.entries(setsByMuscle)) {
          const bucket = DISPLAY_BUCKET[muscle] || muscle;
          if (!byWeek[weekStart][bucket]) byWeek[weekStart][bucket] = { sets: 0, volume: 0 };
          byWeek[weekStart][bucket].sets += sets;
        }
      }
    }
  }
...
//
// DEPRECATED — superseded by anatomy-weighted distribution via
// distributeAcrossMuscles() from exercise-anatomy.js. This function still
// exists because (a) it's used by code paths I haven't migrated yet, and
// (b) it serves as the fallback inside distributeAcrossMuscles when an
// exercise.muscle string is the only signal we have. Once everything reads
// from EXERCISE_ANATOMY, this can be deleted.
//
...
//
// Distributes volume across muscles using anatomy weights — same logic as
// weeklyVolume above. The fatigue baseline comparison is self-consistent
// because both `recent` and `baseline` windows use identical weighting.
function aggregateVolume(records) {
  ...
  for (const rec of records || []) {
    for (const block of rec.blocks || []) {
      for (const ex of block.exercises || []) {
        // Sum total volume for this exercise
        let exVolume = 0;
        for (const s of ex.sets || []) {
          if (s.volume != null) {
            exVolume += s.volume;
        ...
        // Distribute and aggregate to display buckets
        const distributed = distributeAcrossMuscles(ex.name, exVolume, ex.muscle);
        for (const [muscle, vol] of Object.entries(distributed)) {
          const bucket = DISPLAY_BUCKET[muscle] || muscle;
          byMuscle[bucket] = (byMuscle[bucket] || 0) + vol;
        }
        total += exVolume; // total counts the underlying volume once, no double-count
      }
    }
  }
