// lib/tokens.js
// ─────────────────────────────────────────────────────────────────────────────
// Shared design tokens for Project Forge.
// Single source of truth — import into any component that needs styling.
// ─────────────────────────────────────────────────────────────────────────────

export const T = {
  // Background scale (dark to light)
  bg0: "#131110",
  bg1: "#1A1714",
  bg2: "#23201B",
  bg3: "#2D2924",
  bg4: "#38342E",

  // Text scale (light to dark)
  text1: "#EDEBE7",
  text2: "#A09890",
  text3: "#6B6560",
  text4: "#403C38",

  // Accent palette
  coral: "#E0956A",
  sage: "#8BB09A",
  gold: "#C4A882",
  steel: "#A5B8D0",
  rose: "#C9A0B8",

  // Session type themes (main color, dim background, glow)
  strength: { main: "#E0956A", dim: "rgba(224,149,106,0.10)", glow: "rgba(224,149,106,0.18)" },
  zone2:    { main: "#A5B8D0", dim: "rgba(165,184,208,0.10)", glow: "rgba(165,184,208,0.14)" },
  hiit:     { main: "#C9A0B8", dim: "rgba(201,160,184,0.10)", glow: "rgba(201,160,184,0.16)" },
  cardio:   { main: "#A5B8D0", dim: "rgba(165,184,208,0.09)", glow: "rgba(165,184,208,0.12)" },
  rest:     { main: "#6B6560", dim: "rgba(107,101,96,0.08)",  glow: "rgba(107,101,96,0.10)" },

  // Typography
  serif: "var(--font-fraunces), serif",
  sans: "var(--font-dm-sans), sans-serif",

  // Border radii
  r: { sm: 8, md: 14, lg: 20, xl: 28, pill: 999 },

  // Animation easing
  ease: "cubic-bezier(0.22, 1, 0.36, 1)",
};

// Muscle group colours for analytics — 9 display buckets matching the
// DISPLAY_BUCKET map in lib/exercise-anatomy.js. Lower body now splits into
// Quads / Glutes / Hamstrings / Calves (was just "Legs"); upper-arm muscles
// merge into "Arms" (was Biceps + Triceps separately).
//
// Palette discipline: leg muscles share a warm-earth family with luminance
// variation for visual stacking (Calves lightest at top, Hams darkest at
// bottom of a stacked bar). Upper body keeps the original chart accent
// assignments — Chest (coral), Back (sage), Shoulders (steel), Arms (rose),
// Core (warm grey).
export const MUSCLE_COLOURS = {
  // Lower body — warm earth family, luminance varies for stacking clarity
  Quads:      "#C4A882", // warm tan — the original Legs colour, kept for the dominant leg bucket
  Glutes:     "#B8956A", // warmer / darker, sits between Quads and coral
  Hamstrings: "#9C7B5A", // deeper brown — visually "underneath" the quads
  Calves:     "#D4B898", // lightest cream — distinguishes a smaller muscle group

  // Upper body
  Chest:     "#E0956A",  // coral
  Back:      "#8BB09A",  // sage
  Shoulders: "#A5B8D0",  // steel blue
  Arms:      "#C9A0B8",  // rose (combines biceps + triceps)
  Core:      "#A09890",  // warm grey

  // Fallback for unknown muscles
  Other:     "#6B6560",
};
