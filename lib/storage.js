...
// Full sync: pull from blob, merge with local, return authoritative state.
// This is the primary data loading function — blob is canonical.
// Returns { meta, history, source: "blob" | "local" | "merged" } or null on total failure.
export async function syncProfile(profile) {
  ...
  const localHistory = H.get(profile);

  const remote = await blobPull(profile);
  
  // Blob unavailable — fall back to local
  if (!remote) {
    if (localHistory.length || Object.keys(localMeta.weights).length) {
      return { meta: localMeta, history: localHistory, source: "local" };
    }
  ...
  // Merge strategy:
  // - Weights/reps: union of keys, prefer newer values (remote wins ties for now)
  ...
  // Write merged state back to localStorage (write-through cache)
  P.saveWeights(profile, mergedMeta.weights);
  P.saveReps(profile, mergedMeta.reps);
  if (mergedMeta.streak) P.saveStreak(profile, mergedMeta.streak);
  if (mergedMeta.programmeBlock) PB.save(mergedMeta.programmeBlock);
  H.save(profile, mergedHistory);

  // If local had data that remote didn't, push the merge back
  const localHadExtra = localHistory.length > remoteHistory.length ||
    Object.keys(localMeta.weights).length > Object.keys(remoteMeta.weights || {}).length;
  if (localHadExtra) {
    blobPush(profile, { meta: mergedMeta, history: mergedHistory });
  }

  return {
    meta: mergedMeta,
    history: mergedHistory,
    source: localHadExtra ? "merged" : "blob",
  };
}
