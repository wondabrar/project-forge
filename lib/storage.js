 ...
 * @property {"fresh"|"normal"|"cooked"} readiness   Session-level daily readiness (NOT per-set RPE)
...
// progression. Previously this function only handled `easy / hard / limit`
// while the UI ships `easy / normal / cooked`. Default UI RPE is "normal",
// so EVERY set logged with the default option produced rir: null. That bug
// blocked progression across the entire user base — fixed here by aligning
// to the UI's actual 3-point scale.
//
// Mapping rationale:
//   easy   = ≥3 RIR (plenty in the tank — should ADD)
//   normal = 2 RIR (the "default" setting — meaningful effort with margin to spare)
//   cooked = 0 RIR (max effort — should HOLD; also triggers per-session cooked
//            handling at the readiness layer)
//
// Legacy aliases ("hard", "limit") are kept for migration of v1 records
// that may have been logged with those strings before the UI was finalised.
export function rpeToRir(rpe) {
  if (rpe === "easy")   return 3;
  if (rpe === "normal") return 2;
  if (rpe === "hard")   return 1;
  if (rpe === "cooked") return 0;
  // Legacy aliases — pre-launch records used these labels
...
export function rirToRpe(rir) {
  ...
  if (rir >= 1) return "hard";    // surfaced when migrating v1 records that used "hard"
  return "cooked";
}
