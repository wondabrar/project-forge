...
// Zone constraint violations are logged but never block rotation.
export function rotateAccessories(history = {}) {
  ...
  SS_PAIRS.forEach(([ka, kb]) => {
    ...
    if (!sa || !sb) return;
    const za = sa.zone, zb = sb.zone;
    if (za !== zb && !ZONE_ADJ[za]?.includes(zb)) {
      console.warn(`Rotation: zone mismatch ${ka}(${za}) ↔ ${kb}(${zb}) — verify gym layout`);
    }
  });
  return config;
}
...
export const SWAP_DB = {
  "Barbell Back Squat":        [{ name:"Goblet Squat",              eq:"Dumbbell",  muscle:"Quadriceps"         },
                                { name:"Bulgarian Split Squat",     eq:"Dumbbell",  muscle:"Quads & Glutes"     },
                                { name:"Leg Press",                 eq:"Machine",   muscle:"Quads & Glutes"     },
                                { name:"Hack Squat",                eq:"Machine",   muscle:"Quadriceps"         }],
  "Barbell Bench Press":       [{ name:"Dumbbell Bench Press",      eq:"Dumbbell",  muscle:"Chest"              },
                                { name:"Push-Up",                   eq:"Bodyweight",muscle:"Chest"              },
                                { name:"Dumbbell Floor Press",      eq:"Dumbbell",  muscle:"Chest"              }],
  "Barbell Reverse Lunge":     [{ name:"DB Reverse Lunge",          eq:"Dumbbell",  muscle:"Quads & Glutes"     },
                                { name:"Step-Up",                   eq:"Bodyweight",muscle:"Quads & Glutes"     },
                                { name:"Split Squat",               eq:"Bodyweight",muscle:"Quads & Glutes"     }],
  "Chest-Supported DB Row":    [{ name:"Dumbbell Bent-Over Row",    eq:"Dumbbell",  muscle:"Upper back"         },
                                { name:"Cable Row",                 eq:"Cable",     muscle:"Upper back"         },
                                { name:"Resistance Band Row",       eq:"Band",      muscle:"Upper back"         },
                                { name:"TRX Row",                   eq:"Bodyweight",muscle:"Upper back"         }],
  "Barbell Hip Thrust":        [{ name:"Glute Bridge",              eq:"Bodyweight",muscle:"Glutes"             },
                                { name:"Single-Leg Hip Thrust",     eq:"Bodyweight",muscle:"Glutes"             },
                                { name:"Cable Pull-Through",        eq:"Cable",     muscle:"Glutes / Hams"      }],
  "Landmine Press":            [{ name:"Dumbbell Shoulder Press",   eq:"Dumbbell",  muscle:"Shoulders"          },
                                { name:"Arnold Press",              eq:"Dumbbell",  muscle:"Shoulders"          },
                                { name:"Pike Push-Up",              eq:"Bodyweight",muscle:"Shoulders"          }],
  "Hanging Leg Raise":         [{ name:"Lying Leg Raise",           eq:"Bodyweight",muscle:"Core"               },
                                { name:"Ab Wheel",                  eq:"Equipment", muscle:"Core"               },
                                { name:"Reverse Crunch",            eq:"Bodyweight",muscle:"Core"               }],
  "Dead Bug":                  [{ name:"Hollow Body Hold",          eq:"Bodyweight",muscle:"Core / Anti-rot"    },
                                { name:"Plank",                     eq:"Bodyweight",muscle:"Core"               }],
  "Hex Bar Deadlift":          [{ name:"Romanian Deadlift",         eq:"Barbell",   muscle:"Posterior chain"    },
                                { name:"Dumbbell Deadlift",         eq:"Dumbbell",  muscle:"Posterior chain"    },
                                { name:"Sumo Deadlift",             eq:"Barbell",   muscle:"Posterior chain"    }],
  "Barbell Overhead Press":    [{ name:"Dumbbell Shoulder Press",   eq:"Dumbbell",  muscle:"Shoulders"          },
                                { name:"Arnold Press",              eq:"Dumbbell",  muscle:"Shoulders"          },
                                { name:"Push Press",                eq:"Barbell",   muscle:"Shoulders"          }],
  "Leg Press":                 [{ name:"Goblet Squat",              eq:"Dumbbell",  muscle:"Quads & Glutes"     },
                                { name:"Bulgarian Split Squat",     eq:"Dumbbell",  muscle:"Quads & Glutes"     },
                                { name:"Wall Sit",                  eq:"Bodyweight",muscle:"Quadriceps"         }],
  "Pull-Up":                   [{ name:"Lat Pulldown",              eq:"Cable",     muscle:"Lats"               },
                                { name:"Resistance Band Pull-Down", eq:"Band",      muscle:"Lats"               },
                                { name:"TRX Row",                   eq:"Bodyweight",muscle:"Lats"               }],
  "Bulgarian Split Squat":     [{ name:"Reverse Lunge",             eq:"Bodyweight",muscle:"Quads & Glutes"     },
                                { name:"Step-Up",                   eq:"Bodyweight",muscle:"Quads & Glutes"     },
                                { name:"DB Reverse Lunge",          eq:"Dumbbell",  muscle:"Quads & Glutes"     }],
  "Machine Hamstring Curl":    [{ name:"Nordic Curl",               eq:"Bodyweight",muscle:"Hamstrings"         },
                                { name:"Dumbbell Leg Curl",         eq:"Dumbbell",  muscle:"Hamstrings"         },
                                { name:"Swiss Ball Leg Curl",       eq:"Equipment", muscle:"Hamstrings"         }],
  "Copenhagen Plank":          [{ name:"Side Plank",                eq:"Bodyweight",muscle:"Adductors / Core"   },
                                { name:"Lateral Band Walk",         eq:"Band",      muscle:"Adductors"          }],
  "Lateral Raise":             [{ name:"Cable Lateral Raise",       eq:"Cable",     muscle:"Lateral delt"       },
                                { name:"Resistance Band Lateral",   eq:"Band",      muscle:"Lateral delt"       },
                                { name:"Seated Lateral Raise",      eq:"Dumbbell",  muscle:"Lateral delt"       }],
  "Power Clean":               [{ name:"Hang Power Clean",          eq:"Barbell",   muscle:"Full body / explosive" },
                                { name:"Dumbbell Hang Clean",       eq:"Dumbbell",  muscle:"Full body / explosive" },
                                { name:"Kettlebell Swing",          eq:"Kettlebell",muscle:"Posterior chain"    }],
  "DB Walking Lunge":          [{ name:"Reverse Lunge",             eq:"Bodyweight",muscle:"Quads & Glutes"     },
                                { name:"Step-Up",                   eq:"Bodyweight",muscle:"Quads & Glutes"     },
                                { name:"Split Squat",               eq:"Bodyweight",muscle:"Quads & Glutes"     }],
  "Cable Pull-Through":        [{ name:"Good Morning",              eq:"Barbell",   muscle:"Posterior chain"    },
                                { name:"Glute Bridge",              eq:"Bodyweight",muscle:"Glutes"             },
                                { name:"Resistance Band Pull-Through",eq:"Band",    muscle:"Glutes / Hams"      }],
  "Incline DB Press":          [{ name:"Incline Push-Up",           eq:"Bodyweight",muscle:"Upper chest"        },
                                { name:"Landmine Press",            eq:"Barbell",   muscle:"Upper chest"        },
                                { name:"Cable Chest Fly",           eq:"Cable",     muscle:"Upper chest"        }],
  "Seated Cable Row":          [{ name:"Dumbbell Bent-Over Row",    eq:"Dumbbell",  muscle:"Mid back"           },
                                { name:"TRX Row",                   eq:"Bodyweight",muscle:"Mid back"           },
                                { name:"Resistance Band Row",       eq:"Band",      muscle:"Mid back"           }],
  "DB Curl":                   [{ name:"EZ Bar Curl",               eq:"Barbell",   muscle:"Biceps"             },
                                { name:"Resistance Band Curl",      eq:"Band",      muscle:"Biceps"             },
                                { name:"Incline DB Curl",           eq:"Dumbbell",  muscle:"Biceps"             }],
  "Tricep Dips":               [{ name:"Close-Grip Push-Up",        eq:"Bodyweight",muscle:"Triceps"            },
                                { name:"Overhead Tricep Extension", eq:"Dumbbell",  muscle:"Triceps"            },
                                { name:"Resistance Band Pushdown",  eq:"Band",      muscle:"Triceps"            }],
  "Face Pull":                 [{ name:"Resistance Band Face Pull", eq:"Band",      muscle:"Rear delts / cuff"  },
                                { name:"Rear Delt Fly",             eq:"Dumbbell",  muscle:"Rear delts"         },
                                { name:"Y-T-W Raise",               eq:"Bodyweight",muscle:"Rear delts"         }],
  "Low-to-High Cable Crossover":[{ name:"DB Chest Fly",             eq:"Dumbbell",  muscle:"Upper pec / medial" },
                                { name:"Pec Deck",                  eq:"Machine",   muscle:"Chest"              },
                                { name:"Resistance Band Crossover", eq:"Band",      muscle:"Upper pec / medial" }],
};
